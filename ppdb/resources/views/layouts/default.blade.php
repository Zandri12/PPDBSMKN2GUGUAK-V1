...
<script>let elems = Array.prototype.slice.call(document.querySelectorAll('.js-switch'));
    ...
    <script>
        ...
        <script>
            ...
            var nl = button.data('mynl')
            var np = button.data('mynp')
            var jk = button.data('jk')
            var pilihan1 = button.data('pilihan1')
            var pilihan2 = button.data('pilihan2')
            var rata = button.data('rata')
            var agama = button.data('agama')
            var ayah = button.data('ayah')
            var pa = button.data('pa')
            var pka = button.data('pka')
            var ibu = button.data('ibu')
            var pi = button.data('pi')
            var pki = button.data('pki')
            var wali = button.data('wali')
            var pw = button.data('pw')
            var pkw = button.data('pkw')
            var status = button.data('mystatus')
            
            
            var modal = $(this)
           
            modal.find('.modal-body #nl').val(nl)
            modal.find('.modal-body #np').val(np)
            modal.find('.modal-body #jk').val(jk)
            modal.find('.modal-body #agama').val(agama)
            modal.find('.modal-body #pilihan1').val(pilihan1)
            modal.find('.modal-body #pilihan2').val(pilihan2)
            modal.find('.modal-body #rata').val(rata)
            modal.find('.modal-body #ayah').val(ayah)
            modal.find('.modal-body #pa').val(pa)
            modal.find('.modal-body #pka').val(pka)
            modal.find('.modal-body #ibu').val(ibu)
            modal.find('.modal-body #pi').val(pi)
            modal.find('.modal-body #pki').val(pki)
            modal.find('.modal-body #wali').val(wali)
            modal.find('.modal-body #pw').val(pw)
            modal.find('.modal-body #pkw').val(pkw)
})
        </script>
